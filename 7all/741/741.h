...
class SalesData 
{
    friend ostream& operator<<(ostream& os, SalesData& sada);
...
//    SalesData()=default;//要求生成一个默认的最简单的构造函数
    ...
    SalesData(const string &book_note):SalesData(book_note,0,0,0){}
    SalesData(const string &book,const unsigned num,const double sellp,const double salep):book_note_(book),units_sold_(num),sale_price_(sellp),selling_price_(salep)
    {
        if(selling_price_!=0)
            discount_=sale_price_/selling_price_;
    }
...
ostream& operator<<(ostream& os, SalesData& sada)
{
